...
use potentials::static_potentials::{BurkertPotential, LoadablePotential, MiyamotoNagaiPotential, 
    NFWPotential, Potential, PotentialType};
...
/// Load the simulation setup based on an input JSON config file.
...
pub fn load_potential(potential_info: &serde_json::Map<String, serde_json::Value>)
    ...
    match kind {
            PotentialType::NFWPotential => Box::new(NFWPotential::load(potential_params)),
            PotentialType::BurkertPotential => Box::new(BurkertPotential::load(potential_params)),
            PotentialType::MiyamotoNagaiPotential => Box::new(MiyamotoNagaiPotential::load(potential_params)),
        }
    }
